...
func (rrm *relabeledRulesManager) determineManagedBy(ctx context.Context, promRule *monitoringv1.PrometheusRule, alertRuleId string) (string, string) {
	...
	// If generated by AlertingRule CRD, do not mark as operator-managed; treat as user-via-platform
	if alertingRuleOwner(promRule) != "" {
		// leave ruleManagedBy empty
	} else if len(promRule.OwnerReferences) > 0 {
		// Non-AlertingRule ownerReferences imply operator-managed
		ruleManagedBy = managementlabels.ManagedByOperator
	} else if isGitOpsManaged(promRule) {
